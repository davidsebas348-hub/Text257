...
if _G.MAN_ESP then
	...
	if _G.MAN_ESP_DATA then
		for model, highlight in pairs(_G.MAN_ESP_DATA) do
			if highlight then
				pcall(function()
					highlight:Destroy()
				end)
...
local function createESP(manModel)
	if not _G.MAN_ESP then return end
	...
	while _G.MAN_ESP do
		...
		for _, obj in pairs(humFolder:GetDescendants()) do
			if obj:IsA("Model") and obj.Name == "Man1" then
				if obj.Parent and obj.Parent.Name:match("^Man") then
